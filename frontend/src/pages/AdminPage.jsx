...
export default function AdminPage() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      toast.error(e?.response?.data?.detail || "تعذر الإنشاء");
    }
  };
  ...
  const toggleActive = async (u) => {
    try {
      ...
      toast.success(u.active ? "تم إيقاف الحساب" : "تم تفعيل الحساب");
      load();
  ...
  return (
    <div data-testid="admin-page">
      <div className="flex items-end justify-between mb-7 flex-wrap gap-4">
        <div>
          <div className="text-xs uppercase tracking-[0.3em] text-yellow-500/80">إدارة النظام</div>
          <h1 className="font-heading text-4xl font-black mt-2 flex items-center gap-3"><Shield className="text-yellow-500"/> لوحة الإدارة</h1>
          ...
          <tbody>
            {users.map(u => (
              <tr key={u.id} className="border-b border-white/5 hover:bg-white/[0.02]">
                ...
                <td className="py-3 px-4">
                  {u.active ? (
                    <span className="text-[10px] px-2 py-1 rounded bg-emerald-500/15 text-emerald-300">مفعل</span>
                  ) : (
                    <span className="text-[10px] px-2 py-1 rounded bg-rose-500/15 text-rose-300">معطل</span>
                  )}
                </td>
                <td className="py-3 px-4">
                  <button onClick={() => toggleActive(u)} title={u.active ? "إيقاف" : "تفعيل"}
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowForm(false)}>
          <div className="glass-card p-6 max-w-md w-full" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <h2 className="font-heading text-xl font-bold">مستخدم جديد</h2>
            ...
            <form onSubmit={submit} className="space-y-3">
              ...
              <input required type="password" placeholder="كلمة المرور" value={form.password} onChange={e => setForm({...form, password: e.target.value})}
                className="w-full px-4 py-2.5 rounded-lg bg-[#0a0d14]/80 border border-white/10 text-sm"/>
              <input placeholder="المسمى الوظيفي" value={form.title} onChange={e => setForm({...form, title: e.target.value})}
                className="w-full px-4 py-2.5 rounded-lg bg-[#0a0d14]/80 border border-white/10 text-sm"/>
