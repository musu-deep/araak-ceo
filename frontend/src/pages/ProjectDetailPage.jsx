...
export default function ProjectDetailPage() {
  ...
  const submitUpdate = async (e) => {
    ...
    try {
      ...
      load();
    } catch { toast.error("تعذر إضافة التحديث"); }
  };
    ...
    <div data-testid="project-detail">
      ...
      <div className="glass-card p-7 mb-6">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          <div className="text-center">
            <div className="text-xs uppercase tracking-widest text-slate-500 mb-2">نسبة الإنجاز</div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-6 pt-6 border-t border-white/5">
          <Info icon={<Calendar size={14}/>} label="تاريخ الانتهاء" value={project.end_date ? new Date(project.end_date).toLocaleDateString("ar-EG") : "—"}/>
          <Info icon={<DollarSign size={14}/>} label="الميزانية" value={new Intl.NumberFormat("ar-EG").format(project.budget || 0)}/>
      ...
      <div className="flex gap-2 mb-5 border-b border-white/5">
        {["overview", "tasks", "updates"].map(t => (
          <button key={t} onClick={() => setTab(t)}
            ...
            {t === "overview" ? "نظرة عامة" : t === "tasks" ? `المهام (${tasks.length})` : `التحديثات (${updates.length})`}
          </button>
        ))}
      </div>
      ...
      {tab === "overview" && (
        <div className="glass-card p-6">
          <h3 className="font-heading text-lg font-bold mb-4">الخط الزمني للإنجاز</h3>
      ...
      {tab === "tasks" && (
        <div className="glass-card p-6">
          {tasks.length === 0 ? (
            <div className="text-center py-8 text-slate-500">لا توجد مهام مرتبطة</div>
          ) : (
                ...
                <div key={t.id} className="flex items-center justify-between p-4 rounded-lg bg-white/[0.02] border border-white/5">
                  <div>
                    ...
                    <div className="text-xs text-slate-500 mt-1 flex gap-3">
                      ...
                      {t.due_date && <span>الاستحقاق: {new Date(t.due_date).toLocaleDateString("ar-EG")}</span>}
                    </div>
                  </div>
      ...
      {tab === "updates" && (
        <div className="grid lg:grid-cols-3 gap-5">
          <form onSubmit={submitUpdate} className="glass-card p-5 space-y-3">
            ...
            <textarea required placeholder="محتوى التحديث..." value={newUpdate.content} onChange={e => setNewUpdate({...newUpdate, content: e.target.value})}
              className="w-full px-3 py-2 rounded-lg bg-[#0a0d14]/80 border border-white/10 text-sm min-h-[100px]"/>
            <input type="number" min={0} max={100} placeholder="نسبة الإنجاز الجديدة (اختياري)" value={newUpdate.progress} onChange={e => setNewUpdate({...newUpdate, progress: e.target.value})}
              className="w-full px-3 py-2 rounded-lg bg-[#0a0d14]/80 border border-white/10 text-sm"/>
            <button data-testid="submit-update" type="submit" className="w-full py-2.5 rounded-lg bg-yellow-500 text-black font-bold hover:bg-yellow-400">إضافة</button>
          </form>
          <div className="lg:col-span-2 space-y-2">
            {updates.length === 0 ? (
              <div className="glass-card p-8 text-center text-slate-500">لا توجد تحديثات بعد</div>
            ) : updates.map(u => (
