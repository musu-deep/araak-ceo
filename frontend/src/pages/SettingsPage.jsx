...
export default function SettingsPage() {
  ...
  const save = async () => { try { await api.put("/notification-settings", settings); toast.success("تم الحفظ"); } catch { toast.error("تعذر"); } };
  ...
  return (
    <div data-testid="settings-page">
      <div className="mb-7">
        ...
        <h1 className="font-heading text-4xl font-black mt-2">هوية المنصة والتنبيهات</h1>
      </div>

      {/* Themes */}
      <div className="glass-card p-6 mb-5">
        <h2 className="font-heading text-xl font-bold mb-1 flex items-center gap-2"><Palette className="text-yellow-500"/> هوية المنصة</h2>
        <p className="text-sm text-slate-500 mb-5">اختر الثيم المناسب — التغيير يطبق على جميع المستخدمين</p>
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {Object.entries(THEMES).map(([k, t]) => (
            <button key={k} onClick={() => { change(k); toast.success(`تم تطبيق: ${t.name}`); }} data-testid={`theme-${k}`}
              className={`text-right p-1 rounded-xl transition-all ${active === k ? "ring-2 ring-yellow-500" : "ring-1 ring-white/10 hover:ring-white/20"}`}>
      ...
      {settings && (
        <div className="glass-card p-6">
          <h2 className="font-heading text-xl font-bold mb-1 flex items-center gap-2"><Bell className="text-yellow-500"/> إدارة التنبيهات</h2>
          ...
          <div className="space-y-4 mb-6">
            {[
              { k: "in_app_enabled", l: "إشعارات داخل المنصة", d: "تظهر في جرس الإشعارات" },
              { k: "email_enabled", l: "إشعارات البريد الإلكتروني", d: "يتطلب ربط Resend/SendGrid (مفاتيح API)" },
              { k: "whatsapp_enabled", l: "إشعارات واتساب", d: "يتطلب ربط Twilio WhatsApp (مفاتيح API)" },
            ].map(c => (
          ...
          <div className="border-t border-white/5 pt-5">
            <h3 className="font-heading font-bold mb-3">أنواع الأحداث</h3>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
              {[
                { k: "meeting", l: "اجتماعات" }, { k: "meeting_request", l: "طلبات اجتماع" },
